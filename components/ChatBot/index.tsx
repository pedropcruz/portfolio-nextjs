        ...
        <div className={botContentMessage()} key={index}>
          <div className={botName()}>
            <Avatar className={botImageContainer()}>
              <AvatarImage className={botImage()} src="/me.png" alt="chatbotimg" />
              <AvatarFallback>PC</AvatarFallback>
            </Avatar>
            <p>
              Pedro Cruz Assistant <span className={dateTimeLabel()}>{dateTime}</span>
            </p>
          </div>
          {shouldShowMessage &&
            (isLoading ? (
  ...
  return (
    <Card className={card()}>
      <CardHeader className={cardHeader()}>
        ...
        <Button variant="outline" className={closeBtn()} onClick={() => setOpen(false)}>
          <MdClose className={closeBtnIcon()} />
        </Button>
      </CardHeader>
      <CardContent className={cardContent()}>{renderMessages()}</CardContent>
    </Card>
  )
}
