		...
		<DropdownMenu>
			...
			<DropdownMenuContent sideOffset={32} align="end" alignOffset={-64}>
				{languages.map(({ value, label }) => (
					<DropdownMenuItem key={value} className={`${dropdownMenuItem()} ${value === locale ? 'bg-primary-100 text-black dark:bg-white dark:bg-opacity-25 dark:text-white' : ''}`}>
						<Link className={link()} key={value} href={value}>{t(`languages.${label}WithEmoji`)}</Link>
					</DropdownMenuItem>
				))}
			</DropdownMenuContent>
		</DropdownMenu>
